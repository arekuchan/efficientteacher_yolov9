...
def xyn2xy_new(x, w=640, h=640, padw=0, padh=0):
    # Convert normalized segments into pixel segments, shape (n,2)
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = (w * x[:, 0] + padw)+(np.array(x[:, 0] > 0, dtype=np.int32) - 1) # top left x
    y[:, 1] = (h * x[:, 1] + padh)+(np.array(x[:, 1] > 0, dtype=np.int32) - 1)  # top left y
...
# def scale_coords_keypoints(img1_shape, coords, img0_shape, ratio_pad=None):
#     # Rescale coords (xyxy) from img1_shape to img0_shape
#     if ratio_pad is None:  # calculate from img0_shape
#         gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = old / new
#         pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh padding
...
def non_max_suppression_lmk_and_bbox(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, num_points=0, multi_label=False,
    ...
    nc = prediction.shape[2] - 5 - num_points * 2 - 1 # number of classes
    # print('nc:', nc)
    # nc = 2
    # xc = prediction[..., -1]  > conf_thres  # candidates
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # x[:, 5:7] *= x[:, -1:]  # conf = cls_conf * lmk_obj_conf

        # Box (center x, center y, width, height) to (x1, y1, x2, y2)
        box = xywh2xyxy(x[:, :4])
        # box = xywh2xyxy(poly2hbb(x[:, -9:-1]))
        # keypoints = x[:, 7:]
        ...
        if multi_label:
            ...
            # lmk_score, _ = lmk_score.max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), x[:, 5+nc:]), 1)[conf.view(-1) > conf_thres]
            # non_valid_index = x[:, -1]< 0.7
            # non_valid_num = non_valid_index.sum(0)
            # print(x[non_valid_index, 6:-1].shape)
            # x[non_valid_index, -1 - num_points * 2:-1] = torch.zeros((non_valid_num, num_points * 2)).cuda()
        ...
        # x[i] = x[i][x[i][:, 0].argsort(descending=True)] 
        # x[i] = x[i][x[i][:, 0].argsort(descending=False)] 
        output[xi] = x[i]
...
def non_max_suppression_ssod(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, num_points=0, multi_label=False,
                        labels=(), max_det=300):
    """Runs Non-Maximum Suppression (NMS) on inference results
    ...
    # print('nc:', nc, 'num_points:', num_points)
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    # print('nc:', nc)
    # xc = prediction[..., 4] > conf_thres  # candidates
    xc = torch.logical_and(prediction[..., 4] > conf_thres, torch.max(prediction[..., 5:], axis=-1)[0] > conf_thres)  # candidates
...
# def non_max_suppression_bbox(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
#                         labels=(), max_det=300):
def non_max_suppression_bbox(prediction,
                        conf_thres=0.25,
                        iou_thres=0.45,
                        classes=None,
                        agnostic=False,
